...
当前 GUI 还完成了新一轮障碍物摆放与图表预览增强：

1. `GUI -> 避障`
   - 支持多障碍物 `AABB` 切换、新增、删除、读取与写回
   - 支持 `center_mm / size_mm` 步进微调
   - 左侧提供 `XY / XZ / YZ` 三视图辅助预览
   - 右侧提供场景级 `3D` 手动刷新预览，用于查看起始机械臂、目标点、障碍物及膨胀安全包络
2. `GUI -> 图表`
   - 右侧 PNG 预览区支持更大默认显示尺寸
   - 除了按钮式 `放大 / 缩小 / 重置 / 刷新预览` 外，还支持鼠标滚轮缩放与左键拖拽平移
3. `GUI -> 推理 / 避障`
   - 超参数显示标签已改为中文表述，但变量名与后端命令行参数保持不变

当前 GUI 的视觉层也已统一为一版更清晰的轻量工作台风格：

1. 顶部状态区采用统一信息栏
2. 页面分组采用更一致的卡片式面板样式
3. 不改变原有四页功能分工与命令调度逻辑，只提升可读性与层级感
...
当前在界面显示层，这些超参数已经使用中文标签，例如：

1. `肩部分支候选数 / 肘部分支候选数 / 腕部分支候选数`
2. `粗分支最大候选数 / 每个粗分支的细分类候选数 / 子空间最大候选数`
3. `启用 NR 校正 / NR 最大迭代次数 / NR 位置收敛阈值 / NR 姿态收敛阈值 / NR 阻尼系数 / NR 步长缩放`
...
目前该障碍物编辑区还额外配有两类摆放辅助能力：

1. 三视图辅助预览
   - 在同页内查看当前障碍物与机械臂、目标点的平面相对关系
2. 3D 场景预览
   - 手动刷新查看完整摆放场景
   - 当前选中障碍物会高亮显示
