...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Predict hierarchical ABB subspace candidates.")
...
def main() -> None:
    ...
    if min(
        ...
        raise ValueError("All top-k / max-candidate arguments must be >= 1.")
    ...
    if branch_meta["segment_profile"] != fine_meta["segment_profile"]:
        raise ValueError("branch_meta and fine_meta must use the same segment_profile.")
    ...
    x_branch = apply_normalizer(pose.astype(np.float32), branch_mean, branch_std)
    x_fine = apply_normalizer(pose.astype(np.float32), fine_mean, fine_std)
    ...
    branch_candidates = branch_candidates[:args.max_branch_candidates]
    ...
    for branch_label, branch_score in branch_candidates:
        ...
        local_topk = min(args.fine_topk_per_branch, fine_num_classes)
    ...
    final_subspaces = final_subspaces[:args.max_subspace_candidates]
    ...
    result = {
        ...
        "topk": {
            ...
            "fine_per_branch": int(args.fine_topk_per_branch),
        },
        "branch_candidates": [
            {
                "branch_label": int(branch_label),
