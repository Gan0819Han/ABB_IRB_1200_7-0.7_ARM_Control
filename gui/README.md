...
## 当前版本说明
当前 GUI 为第一版可用工具界面，采用 `tkinter` 实现。
...
- 避障页障碍物编辑器已支持位置/尺寸步进微调
- 避障页内置 `XY / XZ / YZ` 三视图辅助预览
- 避障页右侧新增场景级 `3D` 手动刷新预览
- 避障页支持纵向滚动，参数较多时可直接滚动查看下半部分
- 避障成功后自动生成避障图，并在图表页右侧显示预览
- 图表页 PNG 预览支持按钮缩放、滚轮缩放与左键拖拽平移
- 推理页与避障页超参数标签已改为中文显示
- 当前界面已统一为一版轻量工作台风格主题
...
- `步进微调`
  - 可按 `x / y / z / dx / dy / dz` 直接增减位置和尺寸
- `三视图辅助预览`
  - 用于查看障碍物、机械臂与目标点在平面投影下的相对关系
- `3D 场景预览`
  - 位于避障页右侧
  - 用于手动刷新查看起始机械臂、目标点、障碍物与膨胀安全包络
  - 当前选中障碍物会高亮
...
- 若后续需要运行 Python 命令，统一使用 `arm_nn` 环境
